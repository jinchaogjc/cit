...
class DV3PlusMLPHead(DepthwiseSeparableASPPHead):
    def __init__(self,
                 image_size, 
                 num_layers=1,
                 **kwargs):
        super(DV3PlusMLPHead, self).__init__(**kwargs)
        self.MLP = MLP(self.channels, self.channels, self.num_classes, 3)
    ...
    def atm(self, feat):
        pred = self.MLP(feat.flatten(2).transpose(1, 2)).transpose(1, 2)\
                        .reshape(feat.shape[0], self.num_classes, *feat.shape[2:])
        # pred = self.conv_seg(pred)
        return pred
