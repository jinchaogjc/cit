...
from ..lora.layers import Linear as LoraLinear
...
from .aspp_head_lora import ASPPHead
...
class Linear(LoraLinear):
    def __init__(self, *args, r=None, **kwargs):
        r = r if r is not None else R_LINEAR
        super(Linear, self).__init__(*args, r=r, **kwargs)
...
class DV3HeadLoRA(ASPPHead):
    def __init__(self,
                 ...
                 freeze_norm=False,
                 **kwargs):
        super(DV3HeadLoRA, self).__init__(**kwargs)
        if freeze_norm:
        ...
        for i in range(self.use_stages):
            ...
            if use_proj:
                proj = Linear(self.in_channels, dim)
        ...
        self.class_embed = Linear(dim, 1 + 1, r=0)
    ...
    def atm(self, feat):
        ...
        num_cls = q.size()[0]
        attn = attn.view(bs, num_cls, *feat.size()[2:])
        q = q.transpose(0, 1)
