...
class Mask2FormerHead(MaskFormerHead):
    ...
    def __init__(self,
        ...
        if train_cfg:
            self.assigner = MMSEG_TASK_UTILS.build(self.train_cfg['assigner'])
        ...
        if "loss_cls_ignore_range" in kwargs:
            self.ignore_range = kwargs["loss_cls_ignore_range"]
    ...
    def _loss_by_feat_single(self, cls_scores: Tensor, mask_preds: Tensor,
        ...
        # classfication loss
        # shape (batch_size * num_queries, )
        if hasattr(self, "ignore_range"):
            cls_scores = cls_scores[:, self.ignore_range:]
            labels = labels[:, self.ignore_range:]
            label_weights = label_weights[:, self.ignore_range:]
